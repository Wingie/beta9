...
| Auth on `POST /api/v1/workspace` | `pkg/api/v1/workspace.go`, `pkg/api/v1/workspace_auth_test.go` | Upstream registers the route without an auth wrapper, so a request with no token reaches the handler as a plain `echo.Context` and panics on `cc.AuthInfo` (Recover turns it into a 500). Wrapped in `auth.WithClusterAdminAuth`, the check the handler was already attempting. Carried over from the old fork's P0 fixes (#4); the only one whose code survives the rebuild. Upstreamable. |
